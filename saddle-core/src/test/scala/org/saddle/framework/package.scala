...
package object framework {
  ...
  implicit def arbMatrix[S: ScalarTag: Gen]: Arbitrary[Mat[S]] =
    arbMatrixOfN[S](100, 100)

  /** Yield an arbitrary Vector of 100 values. */
  implicit def arbVector[S: ScalarTag: Gen]: Arbitrary[Vec[S]] =
    arbVectorOfN[S](100)

  /** Yield an arbitrary Series of 100 values. */
  implicit def arbSeries[X: Gen: ORD: ScalarTag, S: ScalarTag: Gen]
      : Arbitrary[Series[X, S]] = arbSeriesOfN[X, S](100)

  /** Yield an arbitrary 10 x 100 Frame. */
  implicit def arbFrame[X: Gen: ORD: ScalarTag, S: ScalarTag: Gen]
  ...
  /** Yield an arbitrary 100 x 100 Matrix. */
  def arbMatrixOfN[S: ScalarTag: Gen](cols: Int, rows: Int): Arbitrary[Mat[S]] =
    Arbitrary(genMatrixOfN[S](cols, rows))

  /** Yield an arbitrary Vector of 100 values. */
  def arbVectorOfN[S: ScalarTag: Gen](size: Int): Arbitrary[Vec[S]] =
    Arbitrary(genVectorOfN[S](size))

  /** Yield an arbitrary Series of 100 values. */
  def arbSeriesOfN[X: Gen: ORD: ScalarTag, S: ScalarTag: Gen](
      size: Int
  ): Arbitrary[Series[X, S]] = Arbitrary(genSeriesOfN[X, S](size))

  /** Yield an arbitrary 100 x 100 Series. */
  def arbFrameOfN[X: Gen: ORD: ScalarTag, S: ScalarTag: Gen](
      cols: Int,
      rows: Int
