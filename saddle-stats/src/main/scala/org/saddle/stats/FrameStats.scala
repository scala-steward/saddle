...
/** Statistical methods made available to operate on columns of a Frame via
  * enrichment.
  *
  * @param frame
  *   The frame to enrich
...
class FrameStats[RX, CX, T: ST](frame: Frame[RX, CX, T]) {
  ...
  /** Rolling count; compute count of number of elements in columns of Frame
    * over a sliding window, ignoring any NA values.
    * @param winSz
  ...
  /** Cumulative count for each column; each successive element of the output is
